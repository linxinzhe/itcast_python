...
def compute_bigram_model(text):
    ...
    p_unigrams = dict(unigrams_counter)
    ...
    p_bigrams = {}
    for key, value in bigrams_counter.items():
        if key[0] not in p_bigrams:
            p_bigrams[key[0]] = {key[1]: value}
        else:
            p_bigrams[key[0]][key[1]] = value
...
def test_run():
    ...
    p_unigrams, p_bigrams = compute_bigram_model(TEXT)
